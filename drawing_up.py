...
if __name__ == '__main__':
    ...
    thread_importance = list(map(int, re.sub(r'\s+', '', input()).split(',')))
    staff_names = []
    print(thread_names)
    ...
    for slot in list_slots:
        ...
        staff_priority = list(
            filter(lambda staff: staff_sum[staff] < staff_load[staff],
                   staff_priority))
        # print(staff_priority)
        staff_priority = list(
            filter(lambda staff: not (
                        (slot - 1) in staff_slots[staff] and (slot - 2) in
                        staff_slots[staff]),
                   staff_priority))
        staff_priority = list(
            filter(lambda staff: not (
        ...
        staff_priority = list(
            filter(lambda staff: not (
                        (slot - 1) in staff_slots[staff] and (slot + 1) in
                        staff_slots[staff]),
                   staff_priority))
        if slot % 4 == 0:
            staff_priority = list(
        ...
        threads_priority = sorted(threads_priority,
                                  key=lambda thread: threads_sum[thread])
        threads_priority = sorted(threads_priority,
                                  key=lambda thread: func(thread_names[thread],
                                                          set(staff_priority),
                                                          staff_flows))
        for thread in threads_priority:
            print("thread: ", thread)
            print(thread, staff_priority)
            maybe_twice = []
            staff = None
            for i, staff_x in enumerate(staff_priority):
                if thread_names[thread] in staff_flows[staff_x]:
                    ...
                    if thread_importance[thread] == 2:
                        if staff in staff_priority:
                            staff_priority.remove(staff)
                    else:
                        maybe_twice.append(staff)
                    break
            print("staff:", staff, staff_priority)
